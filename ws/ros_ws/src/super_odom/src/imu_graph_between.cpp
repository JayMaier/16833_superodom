...
gtsam::Pose3 vo_last = gtsam::Pose3(gtsam::Rot3::Quaternion(0.0, \
                                            0.0, 0.0\
                                            , 0.0),
...
class IMU_Graph{
	public:
		...
		void voCallback(const nav_msgs::Odometry &msg){
			...
			gtsam::Pose3 vo_curr = gtsam::Pose3(gtsam::Rot3::Quaternion(vo_msg.orientation.w, \
			...
			if (frame == 0){
                ...
                gtsam::BetweenFactor<gtsam::Pose3> between(X(frame-1), X(frame),vo_delta, pose_correction_noise);
